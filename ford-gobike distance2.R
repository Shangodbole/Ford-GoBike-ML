...
RegionIDs
Stations=merge(Stations,RegionIDs,by="region_id",all.x=T)
...
load("FGB Google API Distance and Times.RData")

Stations=Stations[,!"lat"]
Stations=Stations[,!"lon"]
Stations=Stations[,!"rental_methods"]
Stations=Stations[,!"rental_url"]
Stations=Stations[,!"external_id"]
Stations=Stations[,!"electric_bike_surcharge_waiver"]
Stations=Stations[,!"has_kiosk"]
Stations=Stations[,!"eightd_station_services"]
Stations=Stations[,!"eightd_has_key_dispenser"]
Stations=Stations[,!"region_id"]
Stations=Stations[,!"City"]
mode(Stations[[1]])='numeric'
OriginStations=Stations

colnames(OriginStations)=cc("StartStationID StartStationName StartShortName StartCapacity")
OriginStations=ann(OriginStations)
out[[1]]
OriginStations[[1]]

out=merge(out,OriginStations,all.x=T,by='StartStationID')
colnames(OriginStations)=gsub("Start","End",colnames(OriginStations))
